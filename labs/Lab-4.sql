...
select  o.employeeid, e.firstname, e.lastname, o.orderdate from orders o
inner join employees e on o.employeeid = e.employeeid where o.orderdate =
(select max(oo.orderdate) from orders oo where o.employeeid = oo.employeeid)
order by o.employeeid;
select  o.employeeid, e.firstname, e.lastname, o.orderdate from orders o
inner join employees e on o.employeeid = e.employeeid where o.orderdate =
(select min(oo.orderdate) from orders oo where o.employeeid = oo.employeeid)
 order by o.employeeid;
...
select o.employeeid, o.orderid from orders o where o.orderid in (select od.orderid
  from order_details od where od.productid = 38) order by o.employeeid, o.orderid;

--9. Create a report showing the date of the last sale for every product, ordered by product id
select distinct od.productid, o.orderdate from order_details od join orders o where o.orderedate =
(select max(o2.orderdate)from orders o2 join order_details od2 on o2.orderid = od2.orderid where 
od2.productid = od.productiod) order by od.productid;
